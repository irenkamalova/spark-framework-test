...
public class DoSFilter implements Filter
{
    ...
    private final ConcurrentHashMap<String, DoSFilter.RateTracker> _rateTrackers = new ConcurrentHashMap<>();
    ...
    private DoSFilter.Listener _listener = new DoSFilter.Listener();
    private Semaphore _passes;
    private volatile int _throttledRequests;
    private volatile int _maxRequestsPerSec;
    private Map<DoSFilter.RateType, Queue<AsyncContext>> _queues = new HashMap<>();
    private Map<DoSFilter.RateType, AsyncListener> _listeners = new HashMap<>();
    ...
    {
        for (DoSFilter.RateType rateType : DoSFilter.RateType.values())
        {
            _queues.put(rateType, new ConcurrentLinkedQueue<>());
            _listeners.put(rateType, new DoSFilter.DoSAsyncListener(rateType));
        }
    ...
    protected void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException
    {
        ...
        DoSFilter.RateTracker tracker = (DoSFilter.RateTracker)request.getAttribute(__TRACKER);
        if (tracker != null)
        {
        ...
        final DoSFilter.OverLimit overLimit = tracker.isRateExceeded(System.nanoTime());
        ...
        DoSFilter.Action action = _listener.onRequestOverLimit(request, overLimit, this);
        ...
        switch (action)
        {
            ...
            case DELAY:
                ...
                asyncContext.addListener(new DoSFilter.DoSTimeoutAsyncListener());
                break;
            case THROTTLE:
                throttleRequest(request, response, filterChain, tracker);
                break;
    ...
    private void throttleRequest(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain, DoSFilter.RateTracker tracker) throws IOException, ServletException
    {
        if (LOG.isDebugEnabled())
        ...
        try
        {
            ...
            if (!accepted)
            {
                ...
                {
                    DoSFilter.RateType priority = getPriority(request, tracker);
     ...
     */
    private DoSFilter.RateType getPriority(HttpServletRequest request, DoSFilter.RateTracker tracker)
    {
        if (extractUserId(request) != null)
            return DoSFilter.RateType.AUTH;
        if (tracker != null)
            return tracker.getType();
        return DoSFilter.RateType.UNKNOWN;
    }
     ...
     */
    DoSFilter.RateTracker getRateTracker(ServletRequest request)
    {
        HttpSession session = ((HttpServletRequest)request).getSession(false);

        String loadId = extractUserId(request);
        final DoSFilter.RateType type;
        if (loadId != null)
        {
            type = DoSFilter.RateType.AUTH;
        }
        else
        {
            if (isTrackSessions() && session != null && !session.isNew())
            {
                ...
                type = DoSFilter.RateType.SESSION;
            }
            else
            {
                loadId = isRemotePort() ? createRemotePortId(request) : request.getRemoteAddr();
                type = DoSFilter.RateType.IP;
            }
        }

        DoSFilter.RateTracker tracker = _rateTrackers.get(loadId);

        if (tracker == null)
        {
            boolean allowed = checkWhitelist(request.getRemoteAddr());
            int maxRequestsPerSec = getMaxRequestsPerSec();
            tracker = allowed ? new DoSFilter.FixedRateTracker(_context, _name, loadId, type, maxRequestsPerSec)
                              : new DoSFilter.RateTracker(_context, _name, loadId, type, maxRequestsPerSec);
            tracker.setContext(_context);
            DoSFilter.RateTracker existing = _rateTrackers.putIfAbsent(loadId, tracker);
            if (existing != null)
                tracker = existing;

            if (type == DoSFilter.RateType.IP)
            {
    ...
    static class RateTracker implements Runnable, HttpSessionBindingListener, HttpSessionActivationListener, Serializable
    {
        ...
        public RateTracker(ServletContext context, String filterName, String id, DoSFilter.RateType type, int maxRequestsPerSecond)
        {
            _context = context;
        ...
        public DoSFilter.OverLimit isRateExceeded(long now)
        {
            ...
            {
                return new DoSFilter.RateTracker.Overage(Duration.ofNanos(rate), _maxRequestsPerSecond);
            }
            return null;
        }
        ...
        {
            DoSFilter.RateTracker tracker = (DoSFilter.RateTracker)se.getSession().getAttribute(__TRACKER);
        ...
        public class Overage implements DoSFilter.OverLimit
        {
            ...
            @Override
            public DoSFilter.RateType getRateType()
            {
                return _type;
            }
            ...
            public String toString()
            {
                final StringBuilder sb = new StringBuilder(DoSFilter.OverLimit.class.getSimpleName());
    ...
    private static class FixedRateTracker extends DoSFilter.RateTracker
    {
        public FixedRateTracker(ServletContext context, String filterName, String id, DoSFilter.RateType type, int numRecentRequestsTracked)
        {
    ...
    private class DoSAsyncListener extends DoSFilter.DoSTimeoutAsyncListener
    {
        private final DoSFilter.RateType priority;

        public DoSAsyncListener(DoSFilter.RateType priority)
        {
    ...
    public enum Action
    {
        ...
        {
            if (delayMs < 0)
                return DoSFilter.Action.REJECT;

            if (delayMs == 0)
                return DoSFilter.Action.THROTTLE;

            return DoSFilter.Action.DELAY;
        }
    }

    public interface OverLimit
    {
        DoSFilter.RateType getRateType();
